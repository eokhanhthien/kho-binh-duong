    ...
    public function cms_upuser()
    {
        ...
        if (!empty($users) && count($users) != 0) {
            ...
            foreach ($users as $user) {
                ...
                $html .= '<td class="text-center">' . cms_render_html($user['user_status'], 'user_status', ['fa-unlock', 'fa-lock'], ['Hoạt động', 'Tạm ngừng']) . '</td>';
                 ...
                 if ($this->auth['id']!=$user['id']) { 
                    ...
                    $html .= '<td class="text-center"><i class="fa fa-pencil-square-o edit-item" title="Sửa" onclick="cms_edit_usitem(' . $user['id'] . ')" style="margin-right: 10px; cursor: pointer;"></i><i onclick="cms_del_usitem(' . $user['id'] . ')" title="Xóa" class="fa fa-trash-o delete-item" style="cursor: pointer;"></i></td>';
                     ...
                     $html .= '<td class="text-center"><i class="fa fa-pencil-square-o edit-item" title="Sửa" onclick="cms_edit_usitem(' . $user['id'] . ')" style="margin-right: 10px; cursor: pointer;"></i></td>';
                 }
                $html .= '</tr>';
                ...
                $html .= "<td class='itgroup_name'><div class='group-user'><div class='group-selbox'></div></div><input type=\"password\" id=\"newpassword\" placeholder=\"Reset mật khẩu mới cho nhân viên\" class=\"form-control\" value=\"\"/></td>";
   ...
   public function cms_save_item_user()
    {
        ...
        if ($id == $this->auth['id'] && $data['user_status'] == 0) {
            ...
            if ($data['email'] == $user['email']) {
                $data = $this->cms_common_string->allow_post($data, ['display_name', 'group_id', 'user_status','password']);
                $data['updated'] = gmdate("Y:m:d H:i:s", time() + 7 * 3600);
                if ($data['password']!='') {
                    $new_p=$this->genneral_password_lhson($data['password']);
                    $data['password']=$new_p['pass'];
                    $data['salt']=$new_p['salt'];
                }else{
                    unset($data['password']);
                }
                ...
                if ($this->_check_mail($data['email'])) {
                    $data = $this->cms_common_string->allow_post($data, ['display_name', 'email', 'group_id', 'user_status','password']);
                    $data['updated'] = gmdate("Y:m:d H:i:s", time() + 7 * 3600);
                    if ($data['password']!='') {
                        $new_p=$this->genneral_password_lhson($data['password']);
                        $data['password']=$new_p['pass'];
    ...
    public function genneral_password_lhson($text='')
    {
        
        $salt = $this->cms_common_string->random(69, true);
        $pass=$this->cms_common_string->password_encode($text, $salt);
    ...
    public function cms_selboxgroup($id=0)
    {
        $groups = $this->db->select('id, group_name')->from('users_group')->get()->result_array();
        $users=null;
        if ((int)$id>0) {
            //get customer group by id
            $users = $this->db->from('users')->where('ID',$id)->get()->row_array();
        }
        if (isset($groups) && count($groups)) {
            $html = '';
            ...
            foreach ($groups as $group) {
                ...
                $html .= '<option '.$selected.' value="' . $group['id'] . '">' . $group['group_name'] . '</option>';
            }
            $html .= '</select>';
            echo $this->messages = $html;
        } else {
    ...
    public function cms_del_gritem()
    {
        ...
        $gid = (int)$gid; 
        //tien hanh check user thuoc nhom
        $count = $this->db->from('users')->where('group_id', $gid)->count_all_results();
        if ($count != 0) {
            echo $this->messages="0";
        }else{
     ...
     * CUSTOMER
    /***********************/
    function setPerPageLimitLhson($perpage=10){
        $perpage=(int)$perpage;
        if ($perpage>0) {
            //tien hanh set 
            $_SESSION['perpagelhson']=$perpage;            
        }
    }
