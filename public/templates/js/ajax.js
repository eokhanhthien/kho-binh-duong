...
function cms_save_item_user(id) {
    ...
    var $newpassword = $('#user .table-user tr.edit-tr-item-' + id + ' td.itgroup_name #newpassword').val();
    var $data = {
        'data': {
            'id': id,
...
function cms_save_receipt() {
    ...
    if (total_money.length == 0 || total_money == 0) {
        ...
        var receipt_user = $('#receipt_user').val();
...
function cms_update_receipt($id, $page) {
    ...
    if (total_money.length == 0 || total_money == 0) {
        ...
        var receipt_user = $('#edit_receipt_user').val();
...
function cms_save_orders(type) {
    if ($('tbody#pro_search_append tr').length == 0) {
        ...
        $('tbody#pro_search_append  tr').each(function () {
            ...
            $detail.push({ id: $id, quantity: $quantity, price: $price, discount: 0, note: $notes });
        });
        ...
        $('tbody#pro_gift_search_append  tr').each(function () {
            ...
            $quantity = $(this).find('input.quantity_product_order').val();
            $notes = $(this).find('input.note_product_order').val();
            $detail.push({ id: $id, quantity: $quantity, price: 0, discount: 0, note: $notes });
        });
...
function cms_save_input_return($type, $input_id) {
    if ($('tbody#pro_search_append tr').length == 0) {
        ...
        $('tbody#pro_search_append  tr').each(function () {
            ...
            $price = cms_decode_currency_format($(this).find('input.price-order').val());
            $quantity = $(this).find('input.quantity_product_order').val();
            $detail.push({ id: $id, quantity: $quantity, price: $price, discount: 0 });
        });
    ...
    if ($('#total_receipt_money').val() < 1) {
        ...
        if ($err) {
            ...
            setTimeout(function () {
                ...
                cms_paging_payment_by_customer_id(1);
...
function cms_save_order_return($type, $order_id) {
    if ($('tbody#product_return tr').length == 0) {
        ...
        $('tbody#product_return tr').each(function () {
            ...
            $detail_return.push({ id: $id, return_id: $return_id, quantity: $quantity, price: $price });
        });
        ...
        $('tbody#product_sell tr').each(function () {
            ...
            $detail_sell.push({ id: $id, quantity: $quantity, price: $price });
        });
        if ($('tbody#product_sell tr').length == 0) {
            $data = {
                'return': {
...
function cms_paging_order_by_customer_id($page) {
    ...
    $store_id = $('#payment_store_id').val();
    $date_from = $('#payment-search-date-from').val();
    $date_to = $('#payment-search-date-to').val();
...
function cms_paging_payment_by_customer_id($page) {
    var $ids = $('.tr-item-customer').attr('id');
    var $id = parseInt($ids.replace(/[^\d.]/g, ''));
    if ($id != null)
        $customer_id = $id;
    ...
    var $param = {
        'type': 'POST',
        'url': 'customer/cms_paging_payment_by_customer_id/' + $page,
        'data': $data,
        'callback': function (data) {
            $('.payment-main-body').html(data);
            $('#payment_info').text('Lịch sử thanh toán');
        }
    };
    cms_adapter_ajax($param);
}

function cms_paging_payment_customer_all($page) {
    cms_paging_order_by_customer_id($page);
    cms_paging_payment_by_customer_id($page);
}
